...
import { ChangePasswordDto } from 'src/app/shared/model/change-password-dto';
...
export class ChangePasswordComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    confirmPassword: ['',[Validators.required, Validators.minLength(8), Validators.maxLength(30),Validators.pattern('^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!$_%*?&])([A-Za-z\d$@$!$_%*?&]|[^ ,.#~()-]){8,}$')]],
  })
  }
  ...
  onChangePassword(form:FormGroup):void{
    ...
    this.dto = new ChangePasswordDto(form.value.password, form.value.confirmPassword, this.tokenPassword);
